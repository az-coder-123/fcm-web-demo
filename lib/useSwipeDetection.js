...
export function useSwipeDetection() {
    ...
    useEffect(() => {
        ...
        // When mobile app sends swipeLeft event
        const handleSwipeLeft = (event) => {
            console.log('[useSwipeDetection] swipeLeft from native:', event.detail);
            recordSwipe({
                direction: 'left',
                deltaX: event.detail?.deltaX || 0,
                deltaY: 0,
                velocity: event.detail?.velocity || 0,
                source: 'native',
            });
        ...
        // When mobile app sends swipeRight event
        const handleSwipeRight = (event) => {
            console.log('[useSwipeDetection] swipeRight from native:', event.detail);
            recordSwipe({
                direction: 'right',
                deltaX: event.detail?.deltaX || 0,
                deltaY: 0,
